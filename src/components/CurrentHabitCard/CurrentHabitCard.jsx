...
export default function CurrentHabitCard({id : habitId, name, done, currentSequence, highestSequence, completed, setCompleted}) {
  ...
  const [habitSequence, setHabitSequence] = useState({done, currentSequence, highestSequence})
  ...
  function markAsCompleted (id, habitIsDone){
    ...
    if(habitIsDone){
      ...
      .then(() => {
        ...
        setHabitSequence({done: true, currentSequence : currentSequence , highestSequence : highestSequence})
        habit.done = false
    ...
    axios.post(`${BASE_URL}/${habitId}/check`, {}, config)
      .then(()=>{
        ...
        setHabitSequence({done: true, currentSequence : (currentSequence + 1), highestSequence : highestSequence})
        habit.done = true
    ...
    <HabitCard isCompleted = {done} data-test="today-habit-container">
      <ContentContainer>
        ...
        <ProgressContainer>
          ...
          <Progress  data-test="today-habit-record">Seu recorde: <Highest checked = {habitSequence.highestSequence  === habitSequence.currentSequence && habitSequence.highestSequence !== 0}>{`${highestSequence} ${highestSequence > 1 ? "dias" : "dia"}`}</Highest></Progress>
        </ProgressContainer>
      </ContentContainer>
